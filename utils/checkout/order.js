...
export const getCreateOrderLineItems = (products) => {
  ...
  return products?.map(({ variation_id, product_id, quantity }) => {
    return {
      quantity,
      product_id,
      variation_id,
...
export const createTheOrder = async (
  ...
  // Don't proceed if the previous request has an error.
  ...
  try {
    const request = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/create-order`,
      {
        method: "POST",
        headers: {
    ...
    if (result.error) {
      ...
      // Success: include full order details in the response
      response.success = true;
      response.orderDetails = result;
    }
  } catch (error) {
    console.warn("Handle create order error", error?.message);
    response.error = "Unexpected error occurred while creating the order.";
    setOrderFailedError("Something went wrong. Please try again.");
  }

...
const CartItem = ({ item, products, setCart }) => {
	...
	const handleQtyChange = (event, cartKey, type) => {
		if (typeof window !== 'undefined') {
			...
			if (updatingProduct || removingProduct || (type === 'decrement' && productCount === 1)) {
				return;
			}
			...
			if (products.length) {
				updateCart(item?.key, newQty, setCart, setUpdatingProduct).catch(error => {
			...
			<div className="col-span-2 cart-right-col">
				<div className="flex justify-between flex-col h-full">
					...
					<footer className="cart-product-footer flex justify-between p-4 border-t border-brand-bright-grey">
						...
						{updatingProduct && <img className="woo-next-cart-item-spinner" width="24" src="/cart-spinner.gif" alt="spinner" />}
